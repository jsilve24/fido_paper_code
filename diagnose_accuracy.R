...
load(paste("fitted_models/ME_N",N,"_D",D,"_Q",Q,"_R1.RData",sep=""))
load(paste("fitted_models/MC_N",N,"_D",D,"_Q",Q,"_R1.RData",sep=""))
load(paste("fitted_models/SC_N",N,"_D",D,"_Q",Q,"_R1.RData",sep=""))
load(paste("fitted_models/SU_N",N,"_D",D,"_Q",Q,"_R1.RData",sep=""))
load(paste("fitted_models/CLM_N",N,"_D",D,"_Q",Q,"_R1.RData",sep=""))
#load(paste("fitted_models/CLMU_N",N,"_D",D,"_Q",Q,"_R1.RData",sep=""))
...
Lambda_true <- fit.su$mdataset$Lambda_true[D_ll:D_ul, Q_ll:Q_ul]
...
#CLMU$Lambda <- CLMU$Lambda[D_ll:D_ul, Q_ll:Q_ul,]
...
#CLM$Eta <- CLM$Eta[D_ll:D_ul, N_ll:N_ul,]
#CLMU$Eta <- CLMU$Eta[D_ll:D_ul, N_ll:N_ul,]

mfits <- list("fit.mc"=MC, "fit.me"=ME, "fit.sc"=SC, "fit.su"=SU, "fit.clm"=CLM)
...
cat(paste("MSE CLM:",get_MSE(CLM$Lambda, nrow(CLM$Lambda), ncol(CLM$Lambda), SU$iter, ref),"\n"))
#cat(paste("MSE CLMU:",get_MSE(CLMU$Lambda, nrow(CLMU$Lambda), ncol(CLMU$Lambda), SU$iter, ref),"\n"))
...
#cat(paste("Outside 95 CI count CLMU:",get_outside_count(CLMU$Lambda, ref),"\n"))

if(do_plot) {
  plot_lambda(mfits, Lambda_true=Lambda_true, image_filename=paste(output_dir,"Lambda_subset_plot_N",N,"_D",D,"_Q",Q,"_R1.png",sep=""))
